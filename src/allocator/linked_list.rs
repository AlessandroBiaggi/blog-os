...
impl LinkedListAllocator {
    ...
    fn add_free_region(&mut self, addr: usize, size: usize) {
        // SAFETY: result of mem::align_of is a power of 2
        assert_eq!(unsafe { align_up(addr, mem::align_of::<ListNode>()) }, addr);
    ...
    unsafe fn find_region(&mut self, size: usize, align: usize) -> Option<(&'static mut ListNode, usize)> {
        ...
        while let Some(ref mut region) = current.next {
            ...
            if let Ok(alloc_start) = unsafe { Self::alloc_from_region(&region, size, align) } {
    ...
    /// Requires that `align` is a power of 2
    unsafe fn alloc_from_region(region: &ListNode, size: usize, align: usize) -> Result<usize, ()> {
        // SAFETY: `align` is a power of 2
        let alloc_start = unsafe { align_up(region.start_addr(), align) };
